...
const applyParticleEffect = (
  ...
  const particleOptions = options?.particle || defaultParticle
  
  // Helper function to get a random particle
  const getRandomParticle = () => {
    if (Array.isArray(particleOptions)) {
      return particleOptions[Math.floor(Math.random() * particleOptions.length)]
    }
  ...
  function generateParticle() {
    const particleType = getRandomParticle() // Get a random particle each time
    const size =
    ...
    if (particleType === "circle") {
      ...
      const fontSizeMultiplier = 1.5 // Make emojis 1.5x bigger
