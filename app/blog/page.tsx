        ...
        <div className="grid md:grid-cols-2 gap-6">
          {filtered.map((p) => (
            <article key={p.slug} className="rounded-xl border border-border bg-card p-6 hover:shadow-lg transition-shadow">
              ...
              <div className="text-sm text-muted-foreground" suppressHydrationWarning>{new Date(p.date).toLocaleDateString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric' })} • {p.readingTime}</div>
